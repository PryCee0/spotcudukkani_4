  ...
  products: router({
    ...
    create: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // v8.0: Handle video upload after product creation
        ...
        if (input.videoBase64 && input.videoMimeType) {
          ...
          if (product) {
            await updateProduct(product.id, { videoUrl, videoKey });
          }
        }
    ...
    update: adminProcedure
      ...
      .mutation(async ({ input }) => {
        const { id, imageBase64, imageMimeType, images, existingImages, clearDescription, videoBase64, videoMimeType, removeVideo, ...data } = input;
        ...
        // v8.0: Handle video removal
        if (removeVideo) {
          const existingProduct = await getProductById(id);
          if (existingProduct?.videoKey) {
