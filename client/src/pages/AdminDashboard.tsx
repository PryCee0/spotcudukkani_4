...
export default function AdminDashboard() {
  ...
  // v8.0: Handle video file selection
  const handleVideoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file && file.type.startsWith('video/')) {
      const url = URL.createObjectURL(file);
      setSelectedVideo({ file, preview: url });
    }
    if (videoInputRef.current) {
      videoInputRef.current.value = "";
    }
  ...
  const openEditDialog = (product: any) => {
    ...
    // v8.0: Load existing video
    if (product.videoUrl) {
      setSelectedVideo({ preview: product.videoUrl, isExisting: true });
    } else {
      setSelectedVideo(null);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedVideo?.file) {
      ...
      videoBase64 = videoDataUrl.split(",")[1];
      videoMimeType = selectedVideo.file.type;
    }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    // v8.0: Process video for edit
    let videoBase64: string | undefined;
    let videoMimeType: string | undefined;
    if (selectedVideo?.file && !selectedVideo.isExisting) {
      const reader = new FileReader();
      const videoDataUrl = await new Promise<string>((resolve) => {
        reader.onloadend = () => resolve(reader.result as string);
        reader.readAsDataURL(selectedVideo.file!);
    ...
    updateMutation.mutate({
      ...
      videoBase64,
      videoMimeType,
      removeVideo: !selectedVideo && !videoBase64 ? true : undefined,
    });
  };
          ...
          {selectedVideo ? (
            <div className="relative rounded-xl overflow-hidden bg-black">
              ...
              <Button
                ...
                size="icon"
                variant="ghost"
                className="absolute top-2 right-2 w-8 h-8 bg-red-500/80 hover:bg-red-600 text-white rounded-full"
                onClick={() => setSelectedVideo(null)}
                title="Videoyu kaldır"
              >
                <X className="w-4 h-4" />
              </Button>
              {selectedVideo.isExisting && (
                <div className="absolute bottom-2 left-2 bg-blue-500 text-white text-xs px-2 py-0.5 rounded font-medium">
                  Mevcut
                </div>
              )}
            </div>
            ...
            <div
              ...
              className="border-2 border-dashed rounded-xl p-4 transition-all cursor-pointer border-[#2F2F2F]/20 hover:border-[#FFD300] bg-[#F9F8F4] text-center"
            >
              <Video className="w-8 h-8 mx-auto mb-2 text-[#2F2F2F]/40" />
              <p className="text-sm text-[#2F2F2F]/60 font-medium">Video seçmek için tıklayın</p>
              <p className="text-xs text-[#2F2F2F]/40 mt-1">MP4, WebM (Maks. 50MB)</p>
            </div>
          )}
        </div>
      </div>
